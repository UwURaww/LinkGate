...
const SESSION_WINDOW_MS = 10 * 60 * 1000; // throttle window for starting gate sessions
const SESSION_THRESHOLD = 60; // generous - real visitors on a shared IP shouldn't hit this
...
/** Throttles raw gate-session creation (view-count spam, slug scraping) -
 * separate from the bypass lockout above, and much softer: it just declines
 * extra requests within the window rather than imposing a standing lockout. */
export function isSessionRateLimited(security: SecurityState, ip: string): boolean {
  const entry = security.sessionStarts[ip];
  if (!entry) return false;
  if (Date.now() - entry.firstAt > SESSION_WINDOW_MS) return false;
  return entry.count >= SESSION_THRESHOLD;
}

export function recordSessionStart(security: SecurityState, ip: string): void {
  const now = Date.now();
  const entry = security.sessionStarts[ip];
  if (!entry || now - entry.firstAt > SESSION_WINDOW_MS) {
    security.sessionStarts[ip] = { count: 1, firstAt: now };
    return;
  }
  entry.count += 1;
}
...
export function pruneSecurity(security: SecurityState): void {
  ...
  for (const [ip, entry] of Object.entries(security.sessionStarts)) {
    if (now - entry.firstAt > SESSION_WINDOW_MS) delete security.sessionStarts[ip];
  }
}
